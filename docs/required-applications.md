...
**Status:** Living document — updated as stack evolves
**Date:** 2026-03-09
...
!!! note "Default is always CLI-first"
    A fresh NeXuS install boots to `cage → foot → tmux`. No DE. No panel. No taskbar.
    GUI apps launch on demand via `fzf` launcher. Compositor choice is made during setup — or never, if the user stays CLI.
